...
class TestSchemeValidator(unittest.TestCase):
    ...
    def test_conv_valid(self):
        ...
        errors = SchemeValidator.validate_conv_layer(self.valid_conv_layer,self.conds)
    ...
    def test_conv_no_kernel_size(self):
        ...
        errors = SchemeValidator.validate_conv_layer(conv_layer,self.conds)
        self.assertEqual(len(errors),1)
    ...
    def test_conv_invalid_kernel_size(self):
        def validate(layer, errors_amount):
            self.expected_conds['was_conv']=True    
            errors = SchemeValidator.validate_conv_layer(layer, self.conds)
            self.assertDictEqual(self.conds, self.expected_conds)
            if errors_amount>0:
    ...
    def test_flatten_layer(self):
        self.expected_conds['was_flatten']=True
        errors = SchemeValidator.validate_flatten_layer(self.valid_flatten_layer,self.conds)
        self.assertIs(errors,None)
        self.assertDictEqual(self.conds, self.expected_conds)
    ...
    def test_flatten_with_args(self):
        ...
        errors = SchemeValidator.validate_flatten_layer(flatten_layer,self.conds)
        self.assertEqual(len(errors),1)
        self.assertDictEqual(self.conds, self.expected_conds)

    def test_flatten_after_flatten(self):
        self.expected_conds['was_flatten']=True
        SchemeValidator.validate_flatten_layer(self.valid_flatten_layer,self.conds)
        errors = SchemeValidator.validate_flatten_layer(self.valid_flatten_layer,self.conds)
        self.assertEqual(len(errors),1)
        self.assertDictEqual(self.conds, self.expected_conds)
    ...
    def test_activation_layer(self):
        ...
        errors = SchemeValidator.validate_activation_layer(self.valid_activation_layer,self.conds)
        self.assertIs(errors, None)
        self.assertDictEqual(self.conds, self.expected_conds)

    def test_activation_layer_with_no_args(self):
        self.expected_conds['was_activation']=True
        errors = SchemeValidator.validate_activation_layer({},self.conds)
        self.assertEqual(len(errors), 1)
        self.assertDictEqual(self.conds, self.expected_conds)

    def test_activation_after_activation(self):
        self.expected_conds['was_activation']=True
        SchemeValidator.validate_activation_layer(self.valid_activation_layer,self.conds)
        errors = SchemeValidator.validate_activation_layer(self.valid_activation_layer,self.conds)
        self.assertEqual(len(errors),1)
        self.assertDictEqual(self.conds, self.expected_conds)
    ...
    def test_dense_layer_without_flatten_first(self):
        ...
        errors = SchemeValidator.validate_dense_layer(self.valid_dense_layer,self.conds)
        self.assertEqual(len(errors),1)
        self.assertDictEqual(self.conds, self.expected_conds)
    ...
    def test_validate_batch_normalization_layer(self):
        ...
        errors = SchemeValidator.validate_batch_normalization_layer(self.valid_normalization_layer,self.conds)
        self.assertIs(errors, None)
        self.assertDictEqual(self.conds, self.expected_conds)

    def test_batch_norm_after_batch_norm(self):
        self.expected_conds['was_batch_normalization']=True
        SchemeValidator.validate_batch_normalization_layer(self.valid_normalization_layer,self.conds)
        errors = SchemeValidator.validate_batch_normalization_layer(self.valid_normalization_layer,self.conds)
        self.assertDictEqual(self.conds, self.expected_conds)
        self.assertEqual(len(errors),1)
    ...
    def test_max_pooling_layer(self):
        ...
        errors = SchemeValidator.validate_max_pooling_layer(self.valid_max_pooling_layer,self.conds)
        self.assertIs(errors,None)
        self.assertDictEqual(self.conds, self.expected_conds)

    def test_max_pooling_layer_after_max_pooling_layer(self):
        self.expected_conds['was_pooling']=True
        SchemeValidator.validate_max_pooling_layer(self.valid_max_pooling_layer,self.conds)
        errors = SchemeValidator.validate_max_pooling_layer(self.valid_max_pooling_layer,self.conds)
        self.assertEqual(len(errors),1)
        self.assertDictEqual(self.conds, self.expected_conds)
    ...
    def test_avg_pooling_layer(self):
        self.expected_conds['was_pooling']=True
        errors = SchemeValidator.validate_average_pooling_layer(self.valid_avg_pooling_layer,self.conds)
        self.assertIs(errors,None)
        self.assertDictEqual(self.conds, self.expected_conds)
    ...
    def test_validate_dropout_layer(self):
        ...
        errors = SchemeValidator.validate_dropout_layer(self.valid_dropout_layer,self.conds)
        self.assertIs(errors,None)
        self.assertDictEqual(self.conds, self.expected_conds)

    def test_validate_dropout_layer_after_dropout_layer(self):
        self.expected_conds['was_dropout']=True
        SchemeValidator.validate_dropout_layer(self.valid_dropout_layer,self.conds)
        errors = SchemeValidator.validate_dropout_layer(self.valid_dropout_layer,self.conds)
